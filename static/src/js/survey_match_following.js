...
(function () {
    ...
    // Define our module in a way that works in both Odoo 16 and 17
    var odoo = window.odoo;
    var define = odoo ? odoo.define : function() {};

    // Register our widget with public.widget if available
    define('custom_elearn.match_following', function (require) {
        var publicWidget = require('web.public.widget');
        if (!publicWidget) {
            console.error("Could not load web.public.widget");
            return {};
        }
        
        // Define the match following widget
        publicWidget.registry.SurveyMatchFollowing = publicWidget.Widget.extend({
            selector: '.match_following_container',
            events: {
                'dragstart .o_match_item': '_onDragStart',
        ...
        // Also try to extend survey.form if it exists
        try {
            var SurveyForm = require('survey.form');
            if (SurveyForm && SurveyForm.include) {
                SurveyForm.include({
                    /**
                     * Override to handle match following questions
                     */
            ...
            console.error("Could not extend survey.form:", e);
        }
    ...
    // Add vanilla JavaScript fallback for initialization
    document.addEventListener('DOMContentLoaded', function() {
        console.log("DOM loaded, checking for match following containers");
        var matchContainers = document.querySelectorAll('.match_following_container');
        if (matchContainers.length > 0) {
            console.log("Found match following containers:", matchContainers.length);
        }
    });
})();